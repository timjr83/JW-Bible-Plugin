import { getBible } from "./bibleData";
import { BibleReference, BibleVerseResult, SupportedLang } from "./types";
...
export function getVerseText(
  ...
  const chapterData = getBible(language)[book]?.[chapter];
...
export function insertVerseText(joplin: any, message: any) {
  joplin.commands.execute(
    "insertText",
    message.book + message.chapter + message.verses
  );
}
