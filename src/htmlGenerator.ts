  ...
  for (const ref of references) {
    ...
    itemHtml.push(
      [
        ...
        `      <span class="icon insert-icon" data-book="${
          ref.book
        }" data-chapter="${ref.chapter}" data-verses="${ref.verses.join(
          ","
        )}" title="Insert text to note"></span>`,
        `    </div>`,
        `  </div>`,
        `  <div class="card-body">`,
      ].join("\n")
    ...
    for (const v of verseTexts) {
      const verseHtml = `<span class="versespan"><b>${
        v.verse
      }</b> ${replaceNewlines(
        v.text.startsWith("\r\n") ? v.text.slice(2) : v.text
      )} </span>`;
      if (
        v.text.startsWith("\r\n") &&
